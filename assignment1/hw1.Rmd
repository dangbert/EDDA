...
large). So the original confidence interval was at 98% level, and in our
code above we compute the full confidence interval
...
for (theta in seq(3,12,0.25)){
  cur = doBootstrap(data$After8weeks, theta, makePlot=FALSE)
  if (is.null(results)) {
    results = cur
  } else {
...
# print signficant results
dplyr::filter(results, p>0.05)
#sprintf("res = %d", res)
```
...
The p-value from the sign-test is `r res$p.value` \> 0.05 so there's not
...
The p-value from ANOVA $0.003229 < 0.05$ suggests that diet does have a
...
reject $H_{AB}$, so we need to run ANOVA again with a model that doesnt'
...
**e)**: Given the data, it seems more intuitive to us to use the
