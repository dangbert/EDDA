...
also be from the same distribution.

**e)**

```{r}
data$After8weeks
res = binom.test(sum(data$After8weeks < 6), length(data$After8weeks), p=0.5, alt="g")
res
```

The p-value from the sign-test is \> 0.05 so we fail to reject
$H_0: m < 6$, so the test suggests that the median cholesterol level
after 8 weeks of low fat diet is indeed less than 6.
...
par(mfrow = c(1, 3))
ylim = c(-3, 10)
boxplot(dplyr::filter(data, diet == 1)$weightlost, main="Diet 1", ylab="Weight Lost (kg)", ylim=ylim)
boxplot(dplyr::filter(data, diet == 2)$weightlost, main="Diet 2", ylab="Weight Lost (kg)", ylim=ylim)
boxplot(dplyr::filter(data, diet == 3)$weightlost, main="Diet 3", ylab="Weight Lost (kg)", ylim=ylim)

stripchart(dplyr::filter(data, diet == 1)$weightlost, main="Diet 1", ylab="Weight Lost (kg)", ylim=ylim, vertical=T)
stripchart(dplyr::filter(data, diet == 2)$weightlost, main="Diet 2", ylab="Weight Lost (kg)", ylim=ylim, vertical=T)
stripchart(dplyr::filter(data, diet == 3)$weightlost, main="Diet 3", ylab="Weight Lost (kg)", ylim=ylim, vertical=T)
```
...
```{r}
binom.test(sum(data$weightlost < 0), length(data$weightlost), p=0.5, alt="two.sided")
```

The results of the sign test with p=2.2e-16, lead use to reject the
$H_0: m =0$ (that the median difference in weight before and after the
diet is 0).
