...
cont1 = as.matrix(xtabs(~PClass, data=titanic))
cont1
chisq.test(cont1)

cont2 = as.matrix(xtabs(~Sex, data=titanic))
cont2
chisq.test(cont2)
```
...
evalModel = function(model, name) {
  cat(sprintf("\n%s:\n", name))
  res = summary(coupsglm1)$coefficients
...
<!--# TODO: ask TA if we should include output here? {R, results='hide'} -->
...
```{r}
country1 = data.frame(oligarchy = mean(coups$oligarchy), pollib = factor(0), parties = mean(coups$parties))
country2 = data.frame(oligarchy = mean(coups$oligarchy), pollib = factor(1), parties = mean(coups$parties))
country3 = data.frame(oligarchy = mean(coups$oligarchy), pollib = factor(2), parties = mean(coups$parties))

res = ""
sprintf("country1 prediction = %.3f", predict(coupsglm6, country1, type="response"))
sprintf("country2 prediction = %.3f", predict(coupsglm6, country2, type="response"))
sprintf("country3 prediction = %.3f", predict(coupsglm6, country3, type="response"))
```
