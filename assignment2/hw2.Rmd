...
<!--# TODO: ask TA about interpreting result -->
...
print("Surival rate by class and gender combinations:")
...
plot(titanic$Age,exp(3.759662 + titanic$Age*-0.039177), ylab="Odds of Survival", xlab="Age (years)",main="Survival Odds of a First Class Female vs. Age")

#res$coefficients['(Intercept)', 'Pr(>|z|)']
...
<!--# TODO: larger bin size for barplot -->

<!--# TODO: ask TA how to interpret intercept -->
...
use a threshold of 2, where if the odds were above 2, the subject would
be predicted as survived and not survived otherwise.
...
summary(coupsglm1)$coefficients


#numelec has highest p-value so we remove it
...
print("numelec, numregim, size, popn, pctvote")
...
<!--# TODO: Ask about the factor during step-down -->

### 4 c)

```{r}

country1 = data.frame(oligarchy = mean(coups$oligarchy), pollib = 0, parties = mean(coups$parties))
country2 = data.frame(oligarchy = mean(coups$oligarchy), pollib = 1, parties = mean(coups$parties))
country3 = data.frame(oligarchy = mean(coups$oligarchy), pollib = 2, parties = mean(coups$parties))

predict(coupsglm6,country1, type="response")
predict(coupsglm6,country2, type="response")
predict(coupsglm6,country3, type="response")
